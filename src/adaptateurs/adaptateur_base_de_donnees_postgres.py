...
class AdaptateurBaseDeDonneesPostgres(AdaptateurBaseDeDonnees):
    def __init__(self, nom_base_donnees: Optional[str] = None) -> None:
        ...
        self._connexion = psycopg2.connect(
            host=configuration["HOTE_BDD"],
            database=nom_base_donnees or configuration["NOM_BDD"],
            user=configuration["UTILISATEUR_BDD"],
            password=configuration["MOT_DE_PASSE_BDD"],
            port=configuration["PORT_BDD"],
        )
    ...
    def ajoute_retour_utilisatrice(
        ...
        interaction = Interaction.model_validate(ligne["interaction_evaluee_json"])
