...
def pose_question_gradio(
    ...
    try:
        ...
        if prompt_txt:
            payload["prompt_surcharge"] = prompt_txt
        reponse_client = client.post("/pose_question_dev_debug", json=payload)
...
def cree_interface_gradio(app):
    ...
    with gr.Blocks(title="🔐 Assistant Cyber ANSSI", css=css_cache_footer) as interface:
        ...
        def pose_question_avec_retour_utilisatrice(question: str, prompt_txt: Optional[str]):
