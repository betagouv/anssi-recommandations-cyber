...
name: git
description: |
  Pratiques Git et GitHub du projet MesQuestionsCyber (dépôt betagouv/anssi-recommandations-cyber) :
  nommage des branches, messages de commit, commits atomiques, workflow de contribution.
  DÉCLENCHER dès que l'utilisateur parle de commit, branche, PR, push, merge, ou demande
  comment contribuer au projet.
---

# Pratiques Git — MesQuestionsCyber

Ce skill décrit les conventions en vigueur dans le dépôt `betagouv/anssi-recommandations-cyber`,
extraites de l'historique réel. Les respecter garantit un historique lisible, des revues de
code efficaces, et une intégration sans friction.
...
```
[SECURITE] Mets à jour pip
... suite aux alertes dependabot https://github.com/betagouv/anssi-recommandations-cyber/security/dependabot/131

[SECURITE] Mets à jour idna
... suite à l'alerte dependabot https://github.com/betagouv/anssi-recommandations-cyber/security/dependabot/98

[RENOVATE] Update dependency @types/node to v25.9.5
...
| Catégorie          | Usage                                                             |
|--------------------|-------------------------------------------------------------------|
| `[VERSION]`        | Montée de version du paquet (`pyproject.toml`, `package.json`)    |
...
| `[RENOVATE]`       | Commit produit automatiquement par Renovate                       |

Des catégories métier peuvent aussi apparaître pour regrouper les commits d'une même
fonctionnalité en cours de développement (ex : `[GESTION GUIDES]`).
...
Un commit atomique :
- Compile et passe les tests à lui seul (`pytest` pour le back, `pnpm run test` dans `ui/` pour le front)
