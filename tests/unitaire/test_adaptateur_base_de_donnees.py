...
def obtient_adaptateurs():
    return [AdaptateurBaseDeDonneesEnMemoire] + (
        [] if os.getenv("CI") else [AdaptateurBaseDeDonneesPostgres]
    )


def cree_connexion_admin():
...
def adaptateur_test(request):
    if request.param == AdaptateurBaseDeDonneesEnMemoire:
        yield AdaptateurBaseDeDonneesEnMemoire()

    elif request.param == AdaptateurBaseDeDonneesPostgres:
        nom_bdd_test = f"test_anssi_{str(uuid.uuid4()).replace('-', '_')}"

        conn_admin = cree_connexion_admin()
        conn_admin.autocommit = True
        with conn_admin.cursor() as cursor:
            cursor.execute(
                f"SELECT 1 FROM pg_database WHERE datname = '{nom_bdd_test}'"
            )
        ...
        conn_cleanup = cree_connexion_admin()
...
def test_ajout_retour_utilisatrice_avec_valeurs_nulles_echoue():
    with pytest.raises(
